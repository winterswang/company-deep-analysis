...
# 添加数据源路径
AKSHARE_DOCS_PATH = "/root/.openclaw/workspace/akshare_docs"
XUEQIU_ANALYZER_PATH = "/root/.openclaw/workspace/xueqiu-analyzer-skill/scripts"
LINK_COLLECTOR_PATH = "/root/.openclaw/workspace/link-collector"
...
class DataQueryTools:
    ...
    def query_financial(self, stock_code: str, market: str = "A股", years: int = 5) -> DataResponse:
        """
        财务数据查询
        ...
        try:
            # 导入 akshare_docs
            sys.path.insert(0, AKSHARE_DOCS_PATH)
    ...
    def query_cashflow(self, stock_code: str, market: str = "A股", years: int = 5) -> DataResponse:
        """
        现金流数据查询
    ...
    def query_roic(self, stock_code: str, market: str = "A股", years: int = 5) -> DataResponse:
        """
        ROIC 数据查询
    ...
    def query_xueqiu(
        ...
        try:
            # 导入爬虫
            sys.path.insert(0, '/root/.openclaw/workspace/xueqiu-analyzer-skill/scripts')
            ...
            # 整理讨论数据 (Discussion 是 dataclass，用属性访问)
            discussions = []
            for d in stock_info.discussions:
                discussions.append({
                    'text': d.content if hasattr(d, 'content') else '',
                    'user': d.author if hasattr(d, 'author') else '',
                    'created_at': d.time if hasattr(d, 'time') else '',
                    'comments': d.comments if hasattr(d, 'comments') and d.comments else [],
                })
            
            # 整理资讯数据 (News 是 dataclass)
            news = []
            for n in stock_info.news:
                news.append({
                    'title': n.title if hasattr(n, 'title') else '',
                    'source': n.source if hasattr(n, 'source') else '',
                    'created_at': n.time if hasattr(n, 'time') else '',
                    'content': n.content if hasattr(n, 'content') else '',
                })
            
            # 整理公告数据 (Notice 是 dataclass)
            # v2.1 新增: pdf_link (PDF原文链接), content (公告正文)
            notices = []
            for n in stock_info.notices:
                notice_data = {
                    'title': n.title if hasattr(n, 'title') else '',
                    'created_at': n.time if hasattr(n, 'time') else '',
                    'ai_summary': n.ai_summary if hasattr(n, 'ai_summary') else '',
                }
                # 添加新字段 (v2.1)
                if hasattr(n, 'pdf_link') and n.pdf_link:
                    notice_data['pdf_link'] = n.pdf_link
                if hasattr(n, 'content') and n.content:
                    notice_data['content'] = n.content[:500] if len(n.content) > 500 else n.content
                notices.append(notice_data)
            
            # 整理文章数据 (Article 是 dataclass)
            articles = []
            for a in stock_info.articles:
                articles.append({
                    'text': a.content if hasattr(a, 'content') else '',
                    'user': a.author if hasattr(a, 'author') else '',
                    'created_at': a.time if hasattr(a, 'time') else '',
                })
            ...
            if data_type == "basic":
                ...
                filtered_data = {'discussions': discussions}
            elif data_type == "news":
                filtered_data = {'news': news}
            elif data_type == "notices":
                filtered_data = {'notices': notices}
            elif data_type == "articles":
                filtered_data = {'articles': articles}
            else:
                filtered_data = {
                    'basic': basic_data,
            ...
            # 评估数据质量
            total = len(discussions) + len(news) + len(notices) + len(articles)
    ...
    def retrieve_local(self, query: str, stock_code: str = None, limit: int = 20) -> DataResponse:
        ...
        """
        try:
            sys.path.insert(0, LINK_COLLECTOR_PATH)
